...
function scraper_fn(){
  ...
  scraper.headers = function(value, accessor){
  ...
  scraper.start = function(cb){
    ...
    data_list.forEach(function(datum){
      ...
      that.processRated(url_string + opts, headers, datum, cb);
    })
  }

  scraper.process = function(url, heds, datum, cb, async){
    var that = this;
    var request_options = {
      url: url,
      headers: heds
    ...
    request(request_options, function (err, response, body) {
      ...
      extractorFn(body, datum, function(result){
        ...
        request_numb++;
        result_arr = _.flatten(result_arr);
        if (resultFormat == 'csv') { cb(dsv.csv.format(result_arr)); return false; }
        cb(result_arr);
      }, progress);
    });      
  }
